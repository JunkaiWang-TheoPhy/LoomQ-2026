...
另有像素 RPG 入口 <http://127.0.0.1:8765/pixel.html>。它使用 16-bit 俯视角、草地/河流/桥/树林、角色与底部任务 HUD，玩家在小地图里收集三枚调查碎片，交给 NPC 后启动真实 A/B 量子井。像素地图资产保存在 [`web/assets/pixel-map.png`](web/assets/pixel-map.png)，不承担科学判定。
