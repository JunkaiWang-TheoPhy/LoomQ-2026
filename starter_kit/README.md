...
另有像素 RPG 入口 <http://127.0.0.1:8765/pixel.html>。它使用 16-bit 俯视角、草地/河流/桥/树林、角色、稳定 3:2 游戏舞台、底部调查快捷栏和像素任务 HUD。地图分为雾镜镇、分岔河谷、证据塔庭院三个场景，河水不可通过，只有木桥可以过河；玩家收集三枚调查碎片，交给 NPC 后启动真实 A/B 量子井。像素地图资产保存在 [`web/assets/pixel-map.png`](web/assets/pixel-map.png)，不承担科学判定。
