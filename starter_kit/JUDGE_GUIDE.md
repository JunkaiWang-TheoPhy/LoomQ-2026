...
如果想体验参考图式的像素 RPG，打开 `/pixel.html`。这里是另一张独立小地图：16-bit 俯视角地形、河流和桥、树丛与花、角色/NPC、稳定 3:2 游戏舞台、底部调查快捷栏和像素对话框。用方向键或 WASD 在地图里走，靠近三枚发光碎片按 E 收集，再去找小满启动量子井；量子井会调用真实 `/api/inquiry`，不是假的剧情数字。
