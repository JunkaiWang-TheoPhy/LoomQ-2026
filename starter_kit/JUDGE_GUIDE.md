...
打开 `/game.html`。四件观测站物品各贡献 5 分；全部收集后才能进入分岔原野。留下预测得 10 分，真实 A/B 护照返回后得 20 分并开放证据塔，结论审计得 50 分。总分只记录流程完成度，审计结果仍保留 `supported / unsupported / inconclusive`，不会把错误结论包装成正确答案。

鼠标、触屏和键盘操作使用相同的地点按钮；方向键、WASD 与数字 1/2/3 可在已解锁地点间移动。手机端不依赖地图坐标，地点会变成纵向按钮。
