...
如果想体验参考图式的像素 RPG，打开 `/pixel.html`。这里是另一张独立小地图：16-bit 俯视角地形、河流和桥、树丛与花、角色/NPC、稳定 3:2 游戏舞台、三步新手引导、底部调查快捷栏和像素对话框。地图会提示雾镜镇、分岔河谷、证据塔庭院三个场景；河水不可通过，必须走木桥。PHASE HUD、碎片之间的纠缠轨迹和量子井脉冲是量子风格反馈；点击开始调查后可用右上角音乐开关打开程序化 chiptune 环境音乐。用方向键或 WASD 在地图里走，靠近三枚发光碎片按 E 收集，再去找小满启动量子井；量子井会调用真实 `/api/inquiry`，不是假的剧情数字。
