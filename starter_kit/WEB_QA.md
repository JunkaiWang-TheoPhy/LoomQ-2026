...
像素版额外覆盖：`/pixel.html` 独立路由、稳定 3:2 像素舞台、雾镜镇/分岔河谷/证据塔庭院三个场景、河水不可通行且木桥可通行、16×16 网格移动、墙体碰撞、碎片拾取、NPC/量子井门槛、统一像素 HUD/底部快捷栏、像素地图 PNG 静态资源和触屏控制 DOM；像素地图只负责视觉，不替代 `/api/inquiry` 的证据返回。
