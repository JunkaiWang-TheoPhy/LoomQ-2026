...
本表只比较截止前已由官方机器人归档的固定 commit，不比较赛后分支，也不把“申报”当成“得分”。对手基线为：
...
“覆盖”表示当前仓库有可运行实现和直接验证证据；“外部证据缺口”表示代码路径存在，但没有凭据或新真机 job 时不能诚实伪造实验结果。
...
| 新手 Web 与可视化 | mayloveless React、EndlessTR 引导页、Wayne Web QA | 零依赖响应式 Web；概率柱图+表格；逐门故事；桌面/390px 浏览器验收；无障碍语义 | 功能覆盖；不复制框架栈 |
...
这两项是外部实验材料差距，不是本地软件缺少接口。除此之外，四个固定强对手公开展示的核心软件能力均已有对应实现和回归证据。
